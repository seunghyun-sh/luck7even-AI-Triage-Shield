...
def main() -> None:
    ...
    parser.add_argument("--profile", required=True, help="예: xss-v1 (매니페스트의 payload_profile 값과 일치해야 함)")
    parser.add_argument("--count", type=int, default=DEFAULT_COUNT, help=f"생성할 페이로드 개수 (기본 {DEFAULT_COUNT})")
    parser.add_argument(
        "--force",
        action="store_true",
    ...
    cache = payload_cache.save(cache_path, args.profile, payloads, source="openai", model=model)

    print(f"\n저장 완료: {cache_path} ({len(cache.items)}개)")
    print("계속 진행하기 전에 이 파일을 열어 후보를 검토하세요. 필요하면 항목을 직접 수정/삭제해도 됩니다.")
